...
test('rota de módulo ainda não portado não é nativa', () => {
  assert.strictEqual(ehNativa('/admin/cozinha'), false)
...
test('offline: só as telas nativas ficam clicáveis', () => {
  ...
  const cardapio = h.split('<div class="erailitem').find(p => p.includes('/admin/cozinha'))
  assert.ok(!caixa.includes(' off'), 'Caixa é nativo: continua clicável sem internet')
  assert.ok(cardapio.includes(' off'), 'Cozinha ainda depende da web: esmaecida sem internet')
})
