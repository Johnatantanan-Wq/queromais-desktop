...
// Pedidos tem dois modos: QUADRO (o padrão — é como o balcão trabalha, e é o que o
// painel mostra) e LISTA (a mesma grade das outras telas, para ver tudo de uma vez).
// O quadro já traz a própria faixa de indicadores, busca e filtros.
function htmlPedidos(dados, estado) {
  ...
  if (modo === 'lista') {
    return '<div>' + alternador + htmlListaDaRota('/admin/pedidos', dados, estado) + '</div>'
  }
  return '<div>' + alternador + Quadro.htmlQuadro(dados, estado) + '</div>'
}
